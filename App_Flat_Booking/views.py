...
def My_flat(request, pk):
    ...
    return HttpResponseRedirect(reverse('rent: rent_add'))
...
def add_to_cart(request, pk):
    item = get_object_or_404(Add_Flat, pk=pk)
...
def cart_view(request):
    ...
    if carts.exists() and orders.exists():
        order = orders[0]
        return render(request, 'App_Flat_Booking/cart.html', context={'order': order, 'carts': carts})
    else:
...
def remove_from_cart(request, pk):
    ...
    if order_qs.exists():
        ...
        if order.orderitems.filter(item=item).exists():
            ...
            return redirect('App_Flat_Booking:cart')
        else:
